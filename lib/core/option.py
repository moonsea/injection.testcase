...
def _setMultipleTargets():
    ...
    ### -l Filename ##Load file from logfile
...
def _setGoogleDorking():
    ...
    print "[!]The Google Hacking is repairing"
...
def _setSafeUrl():
    ...
    if not conf.safUrl:
        return
    infoMsg = "The SafeUrl function is repairing."
    logger.info(infoMsg)
...
def _setHTTPAuthentication():
    ...
    if not conf.authType and not conf.authCred and not conf.authPrivate:
        return
    infoMsg = "The authType, authCred, authPrivate function is repairing"
    logger.info(infoMsg)
...
def _setHTTPUserAgent():
    ...
    if conf.mobile:
        infoMsg = "The mobile function is repairing"
        logger.infoMsg(infoMsg)
        ...
        infoMsg = "The agent function is repairing"
        logger.infoMsg(infoMsg)
...
def _setHTTPReferer():
    ...
    if conf.referer:
        infoMsg = "The _setHTTPReferer function is repairing"
        logger.info(infoMsg)
...
def _setHTTPCookies():
    ...
    if conf.cookie:
        infoMsg = "The Cookie Function is repairing"
        logger.info(infoMsg)
...
def _cleanupOptions():
    ...
    for key, value in conf.items():
        ...
        if value and any(key.endswith(_) for _ in ("Path", "File")):
            conf[key] = os.path.expanduser(value)###od.path.expanduser()
...
def _useWizardInterface():
    ...
    logger.info("starting wizard model")

    while not conf.url:
        message = "Please input full target URL (-u):(eg.http://www.almaz-antey.ru/about/exhibition/?year=2014)"
        conf.url = readInput(message, default=None)

    message = "POST data (--data) for POST Method [Enter for None]: "
...
def _resolveCrossReferences():
    ...
    print "-----------------------------------------------------------------------"
    """
    lib.core.threads.readInput = readInput## --answers="xxx=xx" ##get the input from terminal
    """
    print "----------------------------- before getPageTemplate --------------------"
    print "-------------------------  kb ------------------------------------"
    kb_info_file = open("kb_info_file_bofore","w+")
    for key in kb.keys():
        print >> kb_info_file, key,"-------",kb[key]
    print "------------------------------------------------------------------"
    info_file = open("conf_info_before.txt","w+")
    ...
    """
    print "----------------------- After getPageTemplate ----------------------------"
    print "-------------------------  kb ------------------------------------"
    kb_info_file = open("kb_info_file_after","w+")
    for key in kb.keys():
        print >> kb_info_file, key,"-------",kb[key]
    print "------------------------------------------------------------------"
    """
...
def init():
    ...
    ## -x sitemapurl ## parse targets from remote sitemap(xml) file
    ...
    info_file = open("conf_info.txt","w+")
    print "-----------------------  conf ----------------------------------"
    #print configparser
    for key  in conf.keys():
        print >> info_file, key,"------",conf[key]
        #info_file.writelines(''.join([key,'---']))
        #info_file.writelines(''.join([conf(key),'\n']))
    info_file.close()
    print "------------------------------------------------------------------"
    """
